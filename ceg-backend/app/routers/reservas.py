...
async def update_reserva(reserva_id: int, reserva: schemas.ReservaUpdate, db: Session = Depends(get_db)):
    try:
        ...
        if all(getattr(db_reserva, key) == value for key, value in reserva.dict(exclude={'jugadores'}).items()):
            logger.info("No se detectaron cambios en la reserva")
            return db_reserva
